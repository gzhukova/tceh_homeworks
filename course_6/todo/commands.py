...
class StatusCommand(BaseCommand):
    ...
    def change_status(self, objects, status, *args, **kwargs):
        """
        ...
        while True:
            try:
                number = int(input("Введите номер объекта:\n"))
                if number in range(len(objects)):
                    break
            except Exception as ex:
                pass
            print("Введите число от 0 до {}".format(len(objects) - 1))
        objects[number].status = status


class DoneCommand(StatusCommand):

    def perform(self, objects, *args, **kwargs):
        self.change_status(objects, True, *args, **kwargs)
...
class UndoneCommand(StatusCommand):

    def perform(self, objects, *args, **kwargs):
        self.change_status(objects, False, *args, **kwargs)
